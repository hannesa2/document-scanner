package info.hannes.documentscanner.support
...
data class Left<L, R>(
    ...
    override fun filterLeft(filter: (L) -> Boolean, supplier: () -> R) = if (filter(value))
        this
    else
        Right(supplier())
...
data class Right<L, R>(private val value: R) : Either<L, R>() {
    ...
    override fun filterRight(filter: (R) -> Boolean, supplier: () -> L) = if (filter(value))
        this
    else
        Left(supplier())
}
